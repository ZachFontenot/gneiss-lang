...
impl Inferencer {
    ...
    pub fn generalize(&self, ty: &Type) -> Scheme {
        ...
        let generalized = self.generalize_inner(ty, &mut generics, &mut next_generic);
    ...
    fn generalize_inner(
        ...
        match ty.resolve(&self.type_uf) {
            ...
            Type::Arrow { arg, ret } => Type::Arrow {
                arg: Rc::new(self.generalize_inner(&arg, generics, next_generic)),
                ret: Rc::new(self.generalize_inner(&ret, generics, next_generic)),
            },
            Type::Tuple(ts) => Type::Tuple(
                ts.iter()
                    .map(|t| self.generalize_inner(t, generics, next_generic))
                    .collect()
            ),
            Type::Constructor { name, args } => Type::Constructor {
                name,
...
impl Inferencer {
    ...
    pub fn instantiate(&mut self, scheme: &Scheme) -> Type {
        ...
        self.substitute(&scheme.ty, &type_subst)
    }
    ...
    fn substitute(
        ...
        match ty {
            ...
            Type::Arrow { arg, ret } => Type::Arrow {
                arg: Rc::new(self.substitute(arg, type_subst)),
                ret: Rc::new(self.substitute(ret, type_subst)),
            },
            Type::Tuple(ts) => Type::Tuple(
                ts.iter().map(|t| self.substitute(t, type_subst)).collect()
            ),
            Type::Constructor { name, args } => Type::Constructor {
                name: name.clone(),
                args: args.iter().map(|t| self.substitute(t, type_subst)).collect(),
            },
            other => other.clone(),
        }
    }
...
1. **Add `UnionFind`** as a new struct
...
4. **Update `Scheme`** to store lists of generic IDs
5. **Pass `&UnionFind`** to resolve/occurs functions
6. **Convert unify to iterative** using the explicit stack
7. **Remove `TypeVar::Link`** — Union-Find handles linking now
